 ...
 */
EDEPTrajectory::EDEPTrajectory(const TG4Trajectory& trajectory, const std::map<int, std::map<component, std::vector<EDEPHit>>>& hit_map, const TG4PrimaryVertexContainer& primaries) :
  ...
  for (const auto& vertex:primaries) {
    auto primary_trj_it = std::find_if(vertex.Particles.begin(), vertex.Particles.end(), [this](TG4PrimaryParticle primary_trj){return primary_trj.GetTrackId() == this->id_;});
...
std::string EDEPTrajectory::Print(std::string& full_out, int depth, int current_depth) const {
  if (current_depth <= depth) {
    ...
    std::cout <<  this->GetDepth() << " " << this->GetId() << " " << this->GetInteractionNumber() << " " << this << " " << this->GetPDGCode() << std::endl;
...
bool EDEPTrajectory::HasHitNear4DPoint(TLorentzVector point, double distance, double time) const {
  return HasHitWhere([&](const EDEPHit& hit) {
      ...
      return (fabs(hit_point_space_distance) < distance) && 
             (fabs(middle_hit_point_time_distance) < time);
    }
  );
}
