...
namespace sand {
  ...
  class BVH {
    public:
      ...
      BVH(const std::vector<const WireT*> & wires, double max_distance, double overlap_tolerance);
    ...
    private:
      ...
      void fillCellAABBMap(const std::vector< const WireT *> & wires);
   ...
   */
  template <typename WireT>
  BVH<WireT>::BVH(const std::vector<const WireT *> & wires, double max_distance, double overlap_tolerance) {
   ...
   */
  template <typename WireT>
  AABB<WireT>::AABB(const WireT & wire) {
