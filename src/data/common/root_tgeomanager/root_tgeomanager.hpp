...
namespace sand {
   ...
   public:
    class tgeonav : public TGeoNavigator {
     ...
     public:
      ...
      pos_3d get_point() const {
        pos_3d ret;
        ret.SetCoordinates(GetCurrentPoint());
        return ret;
      }
      ...
      inline TGeoHMatrix get_hmatrix() const {
        return *GetCache()->GetCurrentMatrix();
      } // FIXME replace this with xform_3d
  ...
  inline pos_3d root_tgeomanager::tgeonav::to_local(pos_3d master) const {
    ...
    // they test for identity and call memcpy lest the branch predictor get bored. Might have been a good idea in a
    // Pentium II... This is a textbook example of how to best fight the compiler attempts at producing efficient code,
    // might as well write python.
    double m[3];
  ...
  template <typename Func>
  void root_tgeomanager::tgeonav::for_each_node(Func&& f) const {
