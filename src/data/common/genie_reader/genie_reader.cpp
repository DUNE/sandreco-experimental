...
sand::genie_reader& ufw::data::factory<sand::genie_reader>::instance(ufw::context_id i) {
  ...
  for (Long64_t j = spills_boundaries[i].first; j < spills_boundaries[i].second; j++) {
    ...
    reader.stdHeps_.push_back({StdHepN, StdHepPdg, StdHepStatus, StdHepRescat, StdHepX4, StdHepP4, StdHepPolz, StdHepFd,
                               StdHepLd, StdHepFm, StdHepLm});

    if (reader.nuParents_.has_value()) {
      reader.nuParents_->push_back(
          {NuParentPdg, NuParentDecMode, NuParentDecP4, NuParentDecX4, NuParentProP4, NuParentProX4, NuParentProNVtx});
    }
...
void ufw::data::factory<sand::genie_reader>::attach_branches() {
