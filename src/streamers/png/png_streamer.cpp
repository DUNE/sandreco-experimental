...
namespace sand::png {
  ...
  void png_streamer::read(ufw::context_id id) { UFW_FATAL("png_streamer only supports writing."); }
  ...
  void png_streamer::write(ufw::context_id id) {
    ...
    for (const auto& img : m_images->images) {
      auto filename = folder + '/' + basename + '_' + std::to_string(id) + '_' + std::to_string(img.camera_id) + "_T"
                    + std::to_string(long(img.time_begin)) + ext;
      ...
      png_set_IHDR(pngstruct, info, sand::grain::pixel_array<double>::kCols * m_scale_factor,
                   sand::grain::pixel_array<double>::kRows * m_scale_factor, 8, PNG_COLOR_TYPE_GRAY, PNG_INTERLACE_NONE,
                   PNG_COMPRESSION_TYPE_DEFAULT, PNG_FILTER_TYPE_DEFAULT);
      png_set_swap(pngstruct);
      png_write_info(pngstruct, info);
      auto array = img.amplitude_array<uint8_t>();
      UFW_DEBUG("Writing image data for camera {} at {}, pixel average = {}", int(img.camera_id), id,
