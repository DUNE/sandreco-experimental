...
namespace sand {
  ...
  caf::SRVector3D GlucksternSmearing::apply_smearing(const caf::SRLorentzVector& true_p,
                                                     const double intrinsic_pos_res_t, const double intrinsic_pos_res_l,
                                                     const double b_field_magnitude) const {
    // compute path_len/x0 (in this way we isolate the use of ufw tgeomanager in a single function, and not in the
    // constructor, so it is easier to make standalone tests)
    ...
    const auto measure_pt_res =
        compute_measurement_smearing(p_transverse_gev, intrinsic_pos_res_t_m, b_field_magnitude);
    ...
    std::normal_distribution<double> abs_dip_angle_error(0.0, dip_angle_res);
