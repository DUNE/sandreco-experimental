...
namespace sand::fake_reco {
  ...
  void fake_reco::run() {
    ...
    std::size_t childreTrajectoriesCount = inputTree.GetChildrenTrajectories().size();

    UFW_INFO("------------------------------");

    UFW_INFO("Reading root trajectory with id: {}", inputTree.GetId());
    UFW_INFO("Children trajectories foud: {}", childreTrajectoriesCount);

    for (int i = 0; i < childreTrajectoriesCount; ++i) {
