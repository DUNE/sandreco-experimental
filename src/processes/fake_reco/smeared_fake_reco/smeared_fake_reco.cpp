...
namespace sand {
  ...
  smeared_fake_reco::smeared_fake_reco()
    : process{{}, {{"output_caf", "sand::caf::caf_wrapper"}}}, m_edep{nullptr}, m_genie{nullptr}, m_caf{nullptr} {}

  void smeared_fake_reco::configure(const ufw::config& cfg) { process::configure(cfg); }

  void smeared_fake_reco::run() {
  ...
  std::vector<EdepInteractionRange> smeared_fake_reco::make_edep_interaction_map() const {
    const auto& primaries = m_edep->GetChildrenTrajectories();
    if (primaries.empty()) {
  ...
  void smeared_fake_reco::initialize_spill_capacities() {
  ...
  void smeared_fake_reco::process_interaction_particles(::caf::SRTrueInteraction& true_ixn, ::caf::SRInteraction& reco_ixn,
  ...
  void smeared_fake_reco::assert_sizes() const {
    // Truth branch
...
UFW_REGISTER_DYNAMIC_PROCESS_FACTORY(sand::smeared_fake_reco);
