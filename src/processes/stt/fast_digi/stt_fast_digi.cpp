...
namespace sand::stt {
   ...
   *
   * \brief Digitizes simulated energy deposits in STT tubes, simulating detector response and output for wire signals.
   *
  ...
  tracker::digi::signal stt_fast_digi::create_signal(double wire_time, double edep_total, const channel_id& channel) {
    ...
    tracker::digi::signal signal{reco::digi<>{channel, trange, reco::digi<>::source::sim}, wire_time + ran, edep_total};
