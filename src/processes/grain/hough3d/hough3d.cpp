...
#include "cl_src/distance.cl"
  ...
  void hough3d::configure(const ufw::config& cfg) {
    ...
    m_buf_unique_versors.allocate<CL_MEM_COPY_HOST_PTR | CL_MEM_READ_ONLY>(platform.context(), m_unique_versors.size() * sizeof(cl_float3), m_unique_versors.data());
  ...
  void hough3d::run() {
    ...
    for (const auto& ev_points : point_cloud_in.points) {
      ...
      buf_points.allocate<CL_MEM_COPY_HOST_PTR | CL_MEM_READ_WRITE>(platform.context(), cl_points.size() * sizeof(cl_float3), cl_points.data());
